...
document.addEventListener('alpine:init', () => {
    Alpine.data('explorerData', () => ({
        ...
        navigate(view, replaceHistory = false, selectionKey = null, anchor = null) {
            // Prefer path-based URLs (static pages) and let them be progressively enhanced.
            const toStaticPath = (p) => {
                try { if (replaceHistory) window.location.replace(p); else window.location.assign(p); } catch (e) { location.href = p; }
            };
            if (view === 'overview') { toStaticPath('/models/'); return; }
            if (view === 'question_themes') { toStaticPath('/themes/'); return; }
            if (view === 'model_detail') {
                const m = selectionKey || this.selectedModel; if (!m) { console.warn('Missing model key'); return; }
                const safe = this.generateSafeIdForFilename(m);
                toStaticPath(`/models/${safe}/`);
                return;
            }
            if (view === 'question_theme_detail') {
                const k = selectionKey || this.selectedGroupingKey; if (!k) { console.warn('Missing theme key'); return; }
                const safe = this.generateSafeIdForFilename(k);
                const a = anchor ? `#${anchor}` : '';
                toStaticPath(`/themes/${safe}/${a}`);
                return;
            }
            // Keep hash-based navigation for remaining simple views
            let bp = '#/', qp = '';
            if (view === 'model_timeline') {
                bp += 'timeline';
                const p = new URLSearchParams();
                if (this.timelineFilterDomain !== 'all') p.set('domain', this.timelineFilterDomain);
                if (this.timelineFilterCreator !== 'all') p.set('creator', this.timelineFilterCreator);
                if (this.timelineFilterJudgment !== 'pct_complete_overall') p.set('metric', this.timelineFilterJudgment);
                if (this.timelineHighlightCreator !== 'none') p.set('highlight', this.timelineHighlightCreator);
                qp = p.toString();
                ...
                console.warn('Invalid view:', view); bp += 'about';
            }
            const paq = qp ? `${bp}?${qp}` : bp;
            this.internalNavigationInProgress = true;
            if (replaceHistory) { history.replaceState(null, '', paq); } else { history.pushState(null, '', paq); }
            this.parseHash();
            this.$nextTick(() => { this.internalNavigationInProgress = false; });
        },
