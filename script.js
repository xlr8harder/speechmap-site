...
document.addEventListener('alpine:init', () => {
    Alpine.data('explorerData', () => ({
        ...
        initialize() {
            ...
            this.isLoading = true; // Keep loading true until fetch AND first render done
            this.loadingMessage = 'Fetching data...';
            this.errorMessage = null;
            // Set initial view based on hash BEFORE fetching
             this.parseHash(); // Sets initial this.currentView etc.
            this.setupWatchers();
            // Start data load asynchronously
            this.loadData().then(() => {
                // This runs after data is loaded and processed
                this.isDataLoaded = true;
                // Now that data is loaded, parse hash again to potentially set selectedModel/Key
                this.parseHash(true); // Force update based on hash now that filters are ready
                // Initialize table for the CURRENT view now that data is ready
                this.$nextTick(() => {
                    this.isLoading = false; // Hide loading indicator
                    this.initializeTableForView(this.currentView);
                });
            }).catch(error => {
                console.error("Init fetch/process error:", error);
                this.errorMessage = `Failed load: ${error.message}`;
                this.isLoading = false;
            });
            // Add hash listener AFTER initial parse
            window.addEventListener('hashchange', () => this.parseHash());
        },
        // ** NEW: Separate async data loading function **
        async loadData() {
            this.loadingMessage = 'Fetching data...';
            const response = await fetch('us_hard_data.json.gz', { headers: { 'Accept-Encoding': 'gzip' } });
            if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText}`);
            this.loadingMessage = 'Decompressing data...'; await this.$nextTick();
        ...
        // ** RENAMED and Updated handleHashChange **
        parseHash(forceUpdate = false) {
            ...
            // Update state only if needed OR if forced (e.g., after initial load)
            if (forceUpdate || v !== this.currentView || m !== this.selectedModel || k !== this.selectedGroupingKey) {
                 ...
                 // Initialize table AFTER state update if data is loaded
                 if (this.isDataLoaded) {
                      this.$nextTick(() => this.initializeTableForView(this.currentView, anchor));
                 }
            } else { console.log("State matches hash."); if(anchor && this.currentView === 'question_theme_detail') this.smoothScroll('#'+anchor); } // Still scroll if only anchor changed
         },
        // ** MODIFIED: Only update hash, let hash listener handle state **
        navigate(view, replaceHistory = false, selectionKey = null, anchor = null) {
            ...
            const newHash = anchor ? `${baseHash}#${anchor}` : baseHash;
            if (location.hash !== newHash){ console.log(`Push/Replace URL: ${newHash} (repl:${replaceHistory})`); if(replaceHistory) history.replaceState(null,'',newHash); else history.pushState(null,'',newHash); this.parseHash(); /* Trigger update from new hash */ }
            else if (replaceHistory) { // Filter/sort change on same view
                console.log("Same hash nav (filter/sort), ensuring redraw.");
                this.$nextTick(()=>{ if(this.isDataLoaded) this.initializeTableForView(this.currentView); }); // Re-init table
            } else if (anchor && view === 'question_theme_detail') { this.smoothScroll('#'+anchor); } // Scroll to anchor on same page
        },
        ...
        // Helper to initialize correct table based on view
        initializeTableForView(view, anchor = null) {
             console.log(`Initializing table for view: ${view}`);
             this.destroyAllTables(); // Clear any existing tables first
             ...
             // Scroll to anchor if provided, especially after table potentially redraws
             if (anchor && view === 'question_theme_detail') {
                  // Needs a slight delay for DOM updates after potential table init
                  setTimeout(() => this.smoothScroll('#' + anchor), 100);
             }
         },
