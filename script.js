...
document.addEventListener('alpine:init', () => {
    Alpine.data('explorerData', () => ({
        ...
        // Use arrays for model detail filters
        activeModelDomainFilters: [], activeModelVariationFilters: [], activeModelComplianceFilters: [],
        modelDetailSortKey: 'pct_complete', questionThemeDomainFilter: '', // Keep single for theme list filter
        ...
        async initialize() {
            ...
            try {
                ...
                if (!response.ok) throw new Error(`HTTP ${response.status}: ${response.statusText} loading us_hard_data.json.gz`);
                this.loadingMessage = 'Decompressing data...'; await this.$nextTick(); // Allow UI update
                const compressedData = await response.arrayBuffer();
                const decompressedData = pako.inflate(new Uint8Array(compressedData), { to: 'string' });
                this.loadingMessage = 'Parsing data...'; await this.$nextTick();
                const jsonData = JSON.parse(decompressedData);
                this.allResponses = jsonData.records || []; this.complianceOrder = jsonData.complianceOrder || []; if(this.allResponses.length === 0) throw new Error("No records found.");
                this.availableFilters.models = [...new Set(this.allResponses.map(r => r.model))].sort(); this.availableFilters.domains = [...new Set(this.allResponses.map(r => r.domain))].sort(); this.availableFilters.variations = [...new Set(this.allResponses.map(r => r.variation))].sort((a,b)=>parseInt(a)-parseInt(b)); this.availableFilters.grouping_keys = [...new Set(this.allResponses.map(r => r.grouping_key))].sort();
                window.addEventListener('hashchange', () => this.handleHashChange());
                this.handleHashChange(); // Parse initial hash AFTER data is loaded
                this.setupWatchers();
            } catch(e) { console.error("Init error:", e); this.errorMessage = `Failed init: ${e.message}`; this.allResponses = []; }
            finally { this.isLoading = false; this.loadingMessage = ''; console.log("Init complete."); }
        },
        handleHashChange() {
            console.log("Hash Change:", location.hash); const h = location.hash.slice(1); const parts = h.split('#'); /* Separate anchor */ const pathParts = parts[0].split('/').filter(Boolean); const anchor = parts[1] || null; let v='about'; let m=null; let k=null;
            if(pathParts[0]==='overview'){ v='overview'; } else if(pathParts[0]==='model'&&pathParts[1]){ const pM=decodeURIComponent(pathParts[1]); if(this.availableFilters.models.includes(pM)){v='model_detail';m=pM;} else { console.warn(`Invalid model hash: ${pM}`); this.navigate('about', true); return; } } else if(pathParts[0]==='questions'){ if(pathParts[1]){ const pK=decodeURIComponent(pathParts[1]); if(this.availableFilters.grouping_keys.includes(pK)){v='question_theme_detail';k=pK;} else { console.warn(`Invalid key hash: ${pK}`); this.navigate('question_themes', true); return; } } else { v='question_themes'; } }
            if (v !== this.currentView || m !== this.selectedModel || k !== this.selectedGroupingKey) {
                 console.log(`State update: view=${v}, model=${m}, key=${k}`); this.currentView=v; if (v !== 'model_detail') this.selectedModel=null; if (v !== 'question_theme_detail') this.selectedGroupingKey=null; this.selectedModel=m; this.selectedGroupingKey=k;
                 // Clear model detail filters when model changes
                 if (v === 'model_detail') this.clearModelDetailFilters(false); // Don't navigate yet
                 // Use setTimeout to ensure state updates before table init
                 setTimeout(() => { this.destroyAllTables(); console.log("Attempting table init for view:", this.currentView); if(this.currentView==='overview')this.initOverviewTable(); if(this.currentView==='question_themes')this.initQuestionThemesTable(); if(this.currentView==='model_detail')this.initModelDetailTable(); if(anchor) this.smoothScroll('#'+anchor); }, 50); // Slightly longer delay
            } else { console.log("State matches hash."); if(anchor) this.smoothScroll('#'+anchor); } // Scroll if only anchor changed
         },
        navigate(view, replaceHistory = false, selectionKey = null, anchor = null) {
            let baseHash = '#/about'; if (view === 'overview'){ baseHash = '#/overview'; } else if (view === 'question_themes'){ baseHash = '#/questions'; } else if (view === 'model_detail'){ const m = selectionKey || this.selectedModel; if(m) baseHash = `#/model/${encodeURIComponent(m)}`; else return; } else if (view === 'question_theme_detail'){ const k = selectionKey || this.selectedGroupingKey; if(k) baseHash = `#/questions/${encodeURIComponent(k)}`; else return; } else if (view !== 'about'){ console.warn("Invalid view:", view); return; }
            const newHash = anchor ? `${baseHash}#${anchor}` : baseHash; // Append anchor if provided
            if (location.hash !== newHash){ console.log(`URL Update: ${newHash} (repl:${replaceHistory})`); if(replaceHistory) history.replaceState(null,'',newHash); else history.pushState(null,'',newHash); this.handleHashChange(); }
            else if (replaceHistory) { console.log("Same hash nav (filter/sort), ensuring redraw."); setTimeout(()=>{ if(this.currentView==='overview')this.initOverviewTable(); if(this.currentView==='question_themes')this.initQuestionThemesTable(); if(this.currentView==='model_detail')this.initModelDetailTable(); if(anchor) this.smoothScroll('#'+anchor);}, 50); }
            else if (anchor) { this.smoothScroll('#'+anchor); } // Just scroll if hash is same but anchor provided
        },
        selectModel(modelName) { this.selectedModel = modelName; this.navigate('model_detail', false, modelName); },
        // ** MODIFIED: Pass anchor ID **
        selectQuestionTheme(groupingKey, modelAnchorId = null) {
            this.selectedGroupingKey = groupingKey;
            this.navigate('question_theme_detail', false, groupingKey, modelAnchorId); // Pass anchor to navigate
         },

        // ** UPDATED Filter methods **
        filterModelDetailTableByDomain() { this.navigate('model_detail', true); }, // Just trigger nav/redraw
        filterModelDetailTableByVariation() { this.navigate('model_detail', true); },
        filterModelDetailTableByCompliance() { this.navigate('model_detail', true); },
        ...
        // ** Added sorter:"number" to fix sorting **
        ...
        smoothScroll(selector) { /* ... as before ... */ const el=document.querySelector(selector); if(el){ console.log("Scrolling to:", selector); el.scrollIntoView({behavior:'smooth',block:'start'}); } else console.warn("Smooth scroll target not found:",selector); },
        getComplianceBoxStyle(percent) { /* ... as before ... */ let c=COMPLIANCE_COLORS.UNKNOWN; if(typeof percent==='number'&&!isNaN(percent)){c=percent>=90?COMPLIANCE_COLORS.COMPLETE:(percent>=25?COMPLIANCE_COLORS.EVASIVE:COMPLIANCE_COLORS.DENIAL);} const t=(c===COMPLIANCE_COLORS.EVASIVE||c===COMPLIANCE_COLORS.UNKNOWN)?'#333':'white'; return `background-color:${c};color:${t};`; },
...
// ** Global helper needed for model detail cell click **
function generateSafeId(text) { if (!text) return 'id'; let safe_text = String(text).toLowerCase(); safe_text = safe_text.replace(/[^\w\s-]/g, ''); safe_text = safe_text.replace(/\s+/g, '-'); safe_text = safe_text.replace(/-+/g, '-'); safe_text = safe_text.strip ? safe_text.strip('-') : safe_text.replace(/^-+|-+$/g, ''); return safe_text || "id"; }
